...
create table if not exists public.record_requests (
  ...
  status text not null default 'draft' check (
    status in (
      'draft', 'pending-patient-approval', 'approved', 'partially-approved', 'denied',
      'revoked', 'expired', 'awaiting-source-organization', 'source-organization-responded',
      'ready-for-review', 'reviewed', 'completed', 'cancelled'
    )
  ),
...
create or replace function public.can_access_record_request(request_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    select 1
    from public.record_requests request
...
create or replace function public.can_manage_record_request(request_id uuid)
returns boolean
language sql
stable
security definer
...
for insert with check (requesting_practice_id = public.current_provider_practice_id());
...
for select using (public.can_access_record_request(record_request_id));
...
for all using (public.can_manage_record_request(record_request_id))
with check (public.can_manage_record_request(record_request_id));
...
for insert with check (public.can_access_record_request(record_request_id));
...
for all using (public.can_access_record_request(record_request_id))
with check (public.can_access_record_request(record_request_id));
